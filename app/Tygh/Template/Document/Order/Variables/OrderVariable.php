...
class OrderVariable extends GenericVariable implements IActiveVariable
{
    ...
    public function __construct(Context $context, array $config, Formatter $formatter)
    {
		...
		$this->data['area'] = $this->data['shipping'][0]['rate_info']['destination_id'];
		$this->data['area'] = db_get_field("SELECT destination FROM ?:destination_descriptions WHERE lang_code=?s AND destination_id=?i","en",$this->data['area']);
    ...
    public static function attributes()
    {
        return array(
            ...
            'tax_subtotal', 'display_shipping_cost','area', 'secondary_currency', 'coupon_code', 'invoice_header', 'invoice_id_text',
            'products' => array(
                '[0..N]' => array(
