...
function fn_new_checkout_layout_checkout_update_steps_user_exists($cart, $auth, $params, &$redirect_params)
{
    $redirect_params['edit_step'] = $params['update_step'];
}

function fn_new_checkout_layout_checkout_update_steps_pre($cart, $auth, &$params, &$redirect_params)
{
...
function fn_new_checkout_layout_checkout_update_steps_before_update_user_data(&$cart, $auth, $params, $user_id, &$user_data)
{
    if (!$user_id) {
        return;
    }
...
function fn_new_checkout_layout_checkout_update_steps_shipping_changed($cart, $auth, $params, &$redirect_params)
{
 ...
 * @return null|string
 */
function fn_new_checkout_layout_checkout_settings_proxy()
{
    ...
    foreach ($proxied_setting_objects as $subsection_name => $setting_objects) {
        foreach ($setting_objects as $setting_object) {
            ...
            $view->assign('html_id', "addon_option_new_checkout_layout{$setting_object['name']}");
 ...
 */
function fn_new_checkout_layout_get_information()
{
    $is_backed_up = false;
 ...
 */
function fn_new_checkout_layout_import_lite_checkout_layout()
{
    if (Registry::get('addons.new_checkout_layout.status') == 'D') {
        return;
    }
    ...
    $layout_path = sprintf('%s/new_checkout_layout/layouts/%s', Registry::get('config.dir.addons'), $import_filename);
    ...
    if (fn_allowed_for('ULTIMATE')) {
        ...
        foreach ($company_ids as $company_id) {
            ...
            $checkout_location_id = fn_new_checkout_layout_find_checkout_location_id($layout['layout_id']);
    ...
    if ($locations) {
        foreach ($locations as $location) {
            ...
            $new_location_id = fn_new_checkout_layout_find_checkout_location_id($location['layout_id']);
            ...
            fn_new_checkout_layout_update_checkout_blocks($location['company_id'], $new_location_id);
        }
    }

    if (!empty($exported_file_paths)) {
        fn_set_notification('W', __('warning'), __('new_checkout_layout.layout_page_was_backed_up', [
 ...
 */
function fn_new_checkout_layout_restore_checkout_address_position()
{
    $company_ids = [null];
    if (fn_allowed_for('ULTIMATE')) {
        $company_ids = array_merge($company_ids, fn_get_all_companies_ids());
    }

    foreach ($company_ids as $company_id) {
        Settings::instance($company_id)->updateValue('address_position', 'shipping_first', 'Checkout');
    }
}
 ...
 */
function fn_settings_actions_addons_post_new_checkout_layout($status)
{
    if ($status === ObjectStatuses::ACTIVE) {
        fn_set_notification(
            NotificationSeverity::WARNING,
            __('warning'),
            __(
                'new_checkout_layout.layout_can_be_setup',
                [
                    '[href]' => fn_url('addons.update?addon=new_checkout_layout'),
                ]
            )
        );
        ...
        if (fn_new_checkout_layout_has_billing_and_shipping_email_profile_field()) {
            db_query(
        ...
        fn_new_checkout_layout_import_lite_checkout_layout();
        ...
        fn_new_checkout_layout_restore_checkout_address_position();
    }
}
 ...
 */
function fn_new_checkout_layout_update_checkout_blocks($company_id, $checkout_location_id)
{
 ...
 */
function fn_new_checkout_layout_find_checkout_location_id($layout_id)
{
...
function fn_new_checkout_layout_has_billing_and_shipping_email_profile_field()
{
 ...
 */
function fn_new_checkout_layout_checkout_get_user_profiles($auth, &$user_profiles, $profile_fields)
{
...
/**
 * The "checkout_is_terms_and_conditions_agreement_required_pre" hook handler.
 *
 * Actions performed:
 * - Sets Terms and conditions agreement requirement from the corresponding setting.
 *
 * @see \fn_checkout_is_terms_and_conditions_agreement_required()
 */
function fn_new_checkout_layout_checkout_is_terms_and_conditions_agreement_required_pre(&$is_agreement_required)
{
    $is_agreement_required = YesNo::toBool(Registry::get('settings.Checkout.agree_terms_conditions'));
}
