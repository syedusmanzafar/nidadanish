        ...
        {foreach from=$tab item="field" key="field_name"}
           {if $field.type == 'input'}
            ...
            {elseif $field.type == 'title'}
                <h4 class="subheader {if $field.group} grouped hidden {$field.group}{/if} {if $field.show_when}{$prefix}ShowWhen{/if}" id="container_elm_{$field_name}">{__("`$prefix`.title_`$field_name`")}</h4>
