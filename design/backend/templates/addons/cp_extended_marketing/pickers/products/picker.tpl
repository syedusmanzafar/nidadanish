...
{if $type == "links"}
    ...
    {include file="addons/cp_extended_marketing/pickers/products/js.tpl" clone=true product="`$ldelim`product`$rdelim`" root_id=$data_id delete_id="`$ldelim`delete_id`$rdelim`" type="product" position_field=$positions position="0"}
    {if $item_ids}
    {foreach from=$item_ids item="product" name="items"}
        {include file="addons/cp_extended_marketing/pickers/products/js.tpl" product_id=$product product=$product|fn_get_product_name|default:__("deleted_product") root_id=$data_id delete_id=$product|escape:javascript type="product" first_item=$smarty.foreach.items.first position_field=$positions position=$smarty.foreach.items.iteration+$start_pos}
    {/foreach}
    {/if}
    </tbody>
...
{elseif $type == "table"}
    ...
    {foreach from=$item_ids item="product" key="product_id"}
        ...
        {/if}
        {include file="addons/cp_extended_marketing/pickers/products/js.tpl" product=$product_name root_id=$data_id delete_id=$product_id input_name="`$input_name`[`$product_id`]" amount=$product.amount price=$product.price amount_input="hidden" type="options" options=$smarty.capture.product_options options_array=$product.product_options product_id=$product.product_id product_info=$product}
    {/foreach}
    {/if}
    {/hook}
    {include file="addons/cp_extended_marketing/pickers/products/js.tpl" clone=true product="`$ldelim`product`$rdelim`" root_id=$data_id delete_id="`$ldelim`delete_id`$rdelim`" input_name="`$input_name`[`$ldelim`product_id`$rdelim`]" amount="1" amount_input="hidden" type="options" options="`$ldelim`options`$rdelim`" product_id=""}
    </tbody>
    <tbody id="{$data_id}_no_item"{if $item_ids} class="hidden"{/if}>
    <tr class="no-items">
...
<div class="cm-display-radio" id="{$data_id}">
    ...
    <div class="input-append choose-input">
        {include file="addons/cp_extended_marketing/pickers/products/js.tpl" product_id="" holder=$data_id hide_input=$hide_input input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button type="single"}
