        ...
        <div class="ty-product-feature__value">
            ...
            {elseif $feature.feature_type == "ProductFeatures::MULTIPLE_CHECKBOX"|enum && $feature.variants}
                ...
                {foreach from=$feature.variants item="var"}
                    ...
                    {if $var.selected}<li class="ty-product-feature__multiple-item"><span class="ty-compare-checkbox"><i class="ty-compare-checkbox__icon ty-icon-ok"></i></span>{if !$hide_variant_affix}<span class="ty-product-feature__prefix">{$feature.prefix}</span>{/if}{$var.variant}{if !$hide_variant_affix}<span class="ty-product-feature__suffix">{$feature.suffix}</span>{/if}<i></i></li>{/if}
                {/foreach}
                </ul>
