...
{capture name="add_to_cart_fast_`$obj_id`"}
<div class="cm-reload-{$obj_prefix}{$obj_id} {$add_to_cart_class}" id="add_to_cart_update_{$obj_prefix}{$obj_id}">
<a class="ty-btn__primary ty-btn__big ty-btn__add-to-cart ty-fast-cart cm-form-dialog-closer ty-btn" type="submit" href="{fn_url("checkout.fast?obj=`$product.product_id`")}">{__("buy")}</a>
</div>
{/capture}
...
{capture name="buttons_product"}
    {hook name="products:add_to_cart"}
        {if $product.has_options && !$show_product_options && !$details_page}
            ...
            {include file="buttons/add_to_cart.tpl" but_id="button_cart_`$obj_prefix``$obj_id`" but_text=__("add_to_cart") but_href="products.view?product_id=`$product.product_id`" but_role=$opt_but_role but_name="" but_meta="ty-btn__primary ty-btn__big"}
        {elseif $details_page}
            ...
            {include file="buttons/add_to_cart.tpl" but_id=$_but_id but_name="dispatch[checkout.add..`$obj_id`]" but_role=$but_role block_width=$block_width obj_id=$obj_id product=$product but_text=__("add_to_cart_new") but_meta=$add_to_cart_meta}

			<a class="ty-btn__primary ty-btn__big ty-btn__add-to-cart cm-form-dialog-closer ty-btn" type="submit" href="{fn_url("checkout.fast?obj=`$obj_id`")}">{__("add_to_cart")}</a>
			&nbsp;
			{if $extra_button}{$extra_button nofilter}&nbsp;{/if}
        {else}
            {hook name="products:add_to_cart_but_id"}
...
{capture name="old_price_`$obj_id`"}
    {if $show_price_values && $show_old_price}
            ...
            {/hook}
        <!--old_price_update_{$obj_prefix}{$obj_id}-->
    {/if}
{/capture}
{if $no_capture}
...
{capture name="advanced_options_`$obj_id`"}
    {if $show_product_options && $product.has_options}
        <div class="cm-reload-{$obj_prefix}{$obj_id} poroduct_adv_options" id="advanced_options_update_{$obj_prefix}{$obj_id}">
...
{capture name="qty_`$obj_id`"}
    {hook name="products:qty"}
		<div class="cm-reload-{$obj_prefix}{$obj_id} qty_update" id="qty_update_{$obj_prefix}{$obj_id}">
