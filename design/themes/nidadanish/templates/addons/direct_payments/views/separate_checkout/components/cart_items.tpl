                        ...
                        <td class="ty-cart-content__product-elem ty-cart-content__description" style="width: 50%;">
                            {strip}
                                ...
                                {if !$product.exclude_from_calculate}
                                    <a class="{$ajax_class} ty-cart-content__product-delete ty-delete-big" href="{"checkout.delete?vendor_id=`$vendor_id`&cart_id=`$key`&redirect_mode=`$runtime.mode`"|fn_url}" data-ca-target-id="cart_items{$suffix_key},checkout_totals{$suffix_key},cart_status*,checkout_steps{$suffix_key},checkout_cart{$suffix_key}" title="{__("remove")}">&nbsp;<i class="ty-delete-big__icon ty-icon-cancel-circle"></i>
                                    </a>
                                {/if}
                            {/strip}
