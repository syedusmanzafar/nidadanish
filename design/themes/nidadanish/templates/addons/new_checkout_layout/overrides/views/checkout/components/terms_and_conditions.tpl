...
<script type="text/javascript">
    ...
    {if $iframe_mode}
        function fn_check_agreements(suffix) {
            var agreementCount = $('form[name=payments_form_' + suffix + '] input[type=checkbox].cm-agreement').length,
                acceptedAgreementCount = $('form[name=payments_form_' + suffix + '] input[type=checkbox].cm-agreement:checkbox:checked').length,
                checked = agreementCount === acceptedAgreementCount;

            $('#payment_method_iframe_' + suffix).toggleClass('hidden',);
        }
    {/if}
</script>
