{if $field.field_type == $smarty.const.EC_CITY}
    ...
    <div class="ty-control-group ty-profile-field__item ty-{if $section == "S"}shipping{else}billing{/if}-state {$field.class}">
        {if $pref_field_name != $field.description || $field.required == "Y"}
            ...
            <select {if $data_id=="s_city"} id="litecheckout_city"{/if} class="ty-profile-field__select-state cm-city {if $section == "S"}cm-location-shipping{else}cm-location-billing{/if}" data-ca-field-id="{$field.field_id}" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}]" {$disabled_param nofilter} data-ca-city="{$_city}">
...
{if $field.field_type == $smarty.const.EC_DISTRICT}
    ...
    {else if $company_data}
        {$_country = $company_data.country|default:$settings.Checkout.default_country}
        {$_state   = $company_data.state|default:$settings.Checkout.default_state}
        {$_city    = $company_data.city|default:$cities.$_country.$_state.0.city_id}
    {/if}
    {$_district = $value}
    <div class="ty-control-group ty-profile-field__item ty-{if $section == "S"}shipping{else}billing{/if}-state {$field.class}">
        {if $pref_field_name != $field.description || $field.required == "Y"}
            ...
            <select {if $data_id=="s_district"} id="litecheckout_s_district"{/if} class="ty-profile-field__select-state cm-district {if $section == "S"}cm-location-shipping{else}cm-location-billing{/if}" id="{$id_prefix}elm_{$field.field_id}" data-ca-field-id="{$field.field_id}"  name="{$data_name}[{$data_id}]" data-ca-district="{$_district}" {$disabled_param nofilter}>
