...
<div class="ty-step__container{if $edit}-active{/if} ty-step-four" data-ct-checkout="billing_options" id="step_four">
    <h3 class="ty-step__title{if $edit}-active{/if} clearfix">
        ...
        {if $complete && !$edit}
            <a class="ty-step__title-txt" href="{"checkout.checkout?edit_step=step_four&from_step={$cart.edit_step}"|fn_url}" data-ca-target-id="checkout_*">{__("billing_options")}</a>
        {else}
        ...
        <div class="clearfix ty-checkout__billing-tabs">
            ...
            {if $edit}
                {if $cart|fn_allow_place_order:$auth}
                    ...
                    {else}
                        <div class="checkout__block"><h3 class="ty-subheader">{__("text_no_payments_required")}</h3></div>
